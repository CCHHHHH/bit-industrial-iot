...
public class MqttRealtimeSource extends RichParallelSourceFunction<DeviceDataEvent> {
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        if (config.getDataSources() != null) {
            config.getDataSources().forEach(ds -> relevantDeviceIds.add(ds.getDeviceId()));
        }
        ...
        String clientId = "flink-rule-" + config.getRuleId() + "-"
                + getRuntimeContext().getIndexOfThisSubtask() + "-" + UUID.randomUUID().toString().substring(0, 8);
        ...
        mqttClient.setCallback(new MqttCallback() {
            ...
            public void messageArrived(String topic, MqttMessage message) {
                try {
                    ...
                    eventQueue.offer(new DeviceDataEvent(deviceId, pointCode, ts, value, quality));
                } catch (Exception e) {
