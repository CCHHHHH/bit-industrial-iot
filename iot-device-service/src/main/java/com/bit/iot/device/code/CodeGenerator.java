package com.bit.iot.device.code;
...
public class CodeGenerator {
    public static void main(String[] args) {
        ...
        //手动配置数据源
        String url="jdbc:mysql://127.0.0.1:3306/bit_iot?useUnicode=true&autoReconnect=true&characterEncoding=UTF-8&useSSL=false&serverTimezone=GMT%2B8";   //注意修改数据库名
        String name="root";
        String password = "chenhao1998@";


        //数据库表的设置
        List<String> listTable = Arrays.asList("device", "device_catalogue", "device_event", "device_station_code");  // 设置需要自动代码生成的表名
//        List<String> listTableSuffix = Arrays.asList("_b");    //设置 过滤 表的后缀
//        List<String> listTablePrefix = Arrays.asList("fmea_"); //设置 过滤 表的前缀

        //基本信息
        String author = "chenhao";    // 作者
        String parent = "com.bit.iot.system";   // 父包名
        String module = "dhcc.avatar";   //模块包名



        //已封装好，无需更改。可按照需求进行注释
        // String localPath = "C:\\Users\\lenovo\\Desktop\\tempCode";
        //1、配置数据源
...
//                            .other("config")    //自定义包名(一般不在这里生成，而是后面编写的时候自己建包)
                            .pathInfo(Collections.singletonMap(OutputFile.xml, System.getProperty("user.dir") + "/src/main/resources/mapper/com/bit/iot/system"));    // 配置 mapper.xml 路径信息：项目的 resources 目录下
                })
                //4、策略配置
...
//                            .addTablePrefix("fmea_") // 设置 过滤 表的前缀
...
//                            .idType(IdType.AUTO)    //设置主键自增
                ...
                //5、模板配置 - 使用自定义模板生成 OpenAPI 3.0 注解
                .templateConfig(builder -> {
                    builder.disable(TemplateType.ENTITY) // 禁用默认 entity 模板
                            .entity("/templates/entity.java"); // 使用自定义 entity 模板
                })
                //6、模板引擎
